...
前任.skill 是一个可运行在 Claude Code / OpenClaw 的 Skill，也支持通过通用资料包运行在 Cursor、Codex、Gemini CLI 等宿主。
用户通过对话式交互或通用构建器提供原材料（聊天记录 + 照片 + 手动描述），系统自动生成一个可独立运行的前任 Persona Skill / 通用角色资料包。
